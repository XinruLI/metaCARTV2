...
REmrt_GS <- function(mf, maxL, minbucket, minsplit, delQ, lookahead){
  y <- model.response(mf)
  vi <- c(t(mf["(vi)"]))
  ...
  while(delta.Q >= delQ & i <= maxL) {
    ...
    for (pl in nodes) {
      ...
      for (k in 1:nmod) {
        ...
        if (is.numeric(xk)) {
          # NUMERIC VARIABLE
          temp <- re.cutoff(y, vi, xk, pleaf.inx, cnode, minbucket)
          if (is.null(temp)) {
            Dev.new <- -Inf
          } else {
            Dev.new <- temp[2]
          ...
          xk.rank <- rank(tapply(y[pleaf.inx], xk, mean))
          xk.ordinal <- xk.rank[as.character(xk)]
          temp <-  re.cutoff(y, vi, xk.ordinal, pleaf.inx, cnode, minbucket)
          if (is.null(temp)) {
            Dev.new <- -Inf
          } else {
            Dev.new <- temp[2]
