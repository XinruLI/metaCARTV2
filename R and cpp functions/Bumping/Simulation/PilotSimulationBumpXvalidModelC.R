...
Metrics0[i, 2] <- sum(c("x1", "x2") %in% res0$mod)
if (is.null(res0$mod)) {
  Metrics0[i, 3] <- 0
} else {
  Metrics0[i, 3] <- sum(!res0$mod %in% c("x1", "x2"))
}

res1 <- BumpNestingXvalid(g ~ x1 + x2 + x3 + x4, vi = vi, data = dat,
                         maxL  = 8, n.fold = 10, B = 25, c = 1)
Metrics1[i, 1] <- length(res1$n)
Metrics1[i, 2] <- sum(c("x1", "x2") %in% res1$mod)
Metrics1[i, 3] <- sum(!res1$mod %in% c("x1", "x2"))
...
if (maxL == 0) {
  ...
  Metrics2[i, 2] <- sum(c("x1", "x2") %in% mods)
  Metrics2[i, 3] <- sum(! mods %in% c("x1", "x2"))
  }
}
save(Metrics0, Metrics1, Metrics2, file = "BumpXvalidModelCPilot")
print(Sys.time())
Sys.time() - sysTime
