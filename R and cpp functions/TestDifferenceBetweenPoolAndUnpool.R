...
rate.diff <- numeric(100)
rate.diff.head <- numeric(100)
set.seed(1709)
for (i in 1:100){
  print(i)
  K <- sample(40:120, 1)
  ...
  mods <- data.frame(m1 = (x1 > 0.12)*1, m2 = (x2 %in% c("a","c","e"))*1,
                     m3 = x3)
  dat.sim1 <- SimData(c(0,0.8), mods, "~ m1:m2:m3", K, NumGen(n,K), 0)
  dat.sim1 <- data.frame(dat.sim1, x1, x2, x3)
  mf <- PreData(efk~x1+x2+x3, dat.sim1, vi = vark)
  res_unpool <- REmrt_GS_unpool(mf, maxL = 10, 1, 2, F)
  res_pool <- REmrt_GS_cpp(mf,maxL = 10, 1, 2,0, F)
  rate.diff.head[i] <- sum(as.character(res_unpool$tree$mod)[1:6] != as.character(res_pool$tree$mod)[1:6], na.rm = T)/10
  rate.diff[i] <- sum(as.character(res_unpool$tree$mod) != as.character(res_pool$tree$mod), na.rm = T)/10
}
rate.diff
rate.diff.head
mean(rate.diff)
mean(rate.diff.head)
system.time(REmrt_GS_unpool(mf, maxL = 10, 1, 2, F))
system.time(REmrt_GS_cpp(mf,maxL = 10, 1, 2,0, F))
